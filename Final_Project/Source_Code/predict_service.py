...
# detectors (match final_pred settings)
...
def build_white_hand_image_and_pts(cvimg: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[list]]:
    ...
    """
    # hd.findHands expects RGB image; final_pred passed cv2image (RGB) into hd
    img_rgb = cv2.cvtColor(cvimg, cv2.COLOR_BGR2RGB)
    hands_res = hd.findHands(img_rgb, draw=False, flipType=True)
    ...
    image = img_copy[y1:y2, x1:x2]

    white = np.ones((400, 400, 3), np.uint8) * 255
    handz_res = hd2.findHands(cv2.cvtColor(image, cv2.COLOR_BGR2RGB), draw=False, flipType=True)
    handz = handz_res[0] if isinstance(handz_res, tuple) and len(handz_res) > 0 else []
    if not handz:
        return None, None

    hand2 = handz[0]
    pts = hand2['lmList']  # list of 21 (x,y)

    # compute offsets similar to final_pred
    os_ = ((400 - w) // 2) - 15
    os1 = ((400 - h) // 2) - 15
    ...
    for start, end in [(1,2),(2,3),(3,4),(5,6),(6,7),(7,8),(9,10),(10,11),(11,12),(13,14),(14,15),(15,16),(17,18),(18,19),(19,20)]:
        cv2.line(white, (int(pts[start][0] + os_), int(pts[start][1] + os1)), (int(pts[end][0] + os_), int(pts[end][1] + os1)), (0,255,0), 3)
    for conn in [(0,1),(0,5),(5,9),(9,13),(13,17),(17,0),(0,17)]:
        cv2.line(white, (int(pts[conn[0]][0] + os_), int(pts[conn[0]][1] + os1)), (int(pts[conn[1]][0] + os_), int(pts[conn[1]][1] + os1)), (0,255,0), 3)
    for i in range(21):
        cv2.circle(white, (int(pts[i][0] + os_), int(pts[i][1] + os1)), 3, (0,0,255), -1)
